...
# 1. 첫 실행이면 venv 부트스트랩 (vllm + 의존성 설치).
if [ ! -d .venv ]; then
...
# 2. 이미 떠 있으면 부팅 스킵 (안전망 — 같은 세션에서 연속 호출 시).
...
# 3. /v1/models 가 200 응답할 때까지 폴링 (최대 STARTUP_TIMEOUT_SEC).
...
# 4. ready 신호 작성 후 종료. 추론은 클라이언트가 ${VLLM_BASE_URL}/chat/completions 로.
